...
public class Node {
    private final int x; //Координата x узла на карте
    private final int y; //Координата y узла на карте
    ...
    public static <T> List<Coordinates> aStar(Coordinates start, Coordinates end, MapEntity map, Class<?> obstacle){
        Node startNode = null;
        Node endNode = null;

        if (start != null && end != null) {
            startNode = new Node(start.getX(), start.getY());
            endNode = new Node(end.getX(), end.getY());
        }
        ...
        while (!queue.isEmpty()){
            ...
            for (int dx = -1; dx < 2; dx++) {
                for (int dy = -1; dy < 2; dy++) {
                    ...
                    if ((tmp = map.getEntity(new Coordinates(x, y))) != null && !(obstacle.isInstance(tmp))) continue;
